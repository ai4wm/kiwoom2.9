...
public:
	...
	CComboBox				m_cbAcctRate;		// 보유계좌처리
	CComboBox				m_cbExchange;		// 거래소
	...
	double					m_dComm1;// 코스피
	double					m_dComm2;// 코스닥
	double					m_dComm3;// 농어촌특별세
	double					m_dComm4;// K-OTC
	// 그리드 데이타 설정(주식기본정보 설정)
	void SetDataRealAddGrid(CStringArray &arrData, CString strRealType = "");
	CString				m_strExchageCode; // 거래소별 종목코드 구성, 통합:_AL, KRX:공백, NXT:_NX, 039490_AL, 039490, 039490_NX
	void				GetExchangeType(CString &rType);/// 거래소 구분값
// 구현입니다.
protected:
	HICON m_hIcon;
	...
	afx_msg void OnSelChAcct();
	afx_msg void OnSelChExchange();
	DECLARE_MESSAGE_MAP()
};
