...
const stGRID lstOPT10001[] = 
{
	...
	{"EPS",				"-1",	0,	9,	DT_NUMBER_SIGN,	FALSE,	DT_RIGHT,	"",	""}, 
	/////////////////////////////////////
	// 주식기본정보 그리드 2행
	...
	{"BPS",				"-1",	1,	9,	DT_NUMBER_SIGN,	FALSE,	DT_RIGHT,	"",	""}, 
	/////////////////////////////////////
	// 주식기본정보 그리드 3행
	...
	{"PER",				"-1",	3,	9,	DT_NUMBER_SIGN,	FALSE,	DT_CENTER,	"",	""}, 
	/////////////////////////////////////
	// 주식기본정보 그리드 5행
...
const stGRID lstOPT10003[] = 
{
	...
	{"체결거래량",		"15",	-1,	4,	DT_ZERO_NUMBER,	TRUE,	DT_RIGHT,	"",	""}, 
	{"체결강도",	"228",	-1,	5,	DT_ZERO_NUMBER,	FALSE,	DT_RIGHT,	"",	""}, 
};
...
const stGRID lstOPT10004[] = 
{
	...
	{"매수최우선잔량",	"71",	11,	2,	DT_NUMBER,		FALSE,	DT_RIGHT,	"",	""}, 
	...
	: CDialogEx(CCurrentPriceDlg::IDD, pParent)
{
	...
	m_nRealPause = 0;/// 종목 실시간 시세 수신여부 0:수신, 1:Pause
}
...
BEGIN_MESSAGE_MAP(CCurrentPriceDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_REALPAUSE, OnBtnRealPause)
END_MESSAGE_MAP()
...
BOOL CCurrentPriceDlg::OnInitDialog()
{
	...
	((CEdit*)GetDlgItem(IDC_EDT_JONGCODE))->SetLimitText(9);	// 종목코드 최대길이 설정
...
void CCurrentPriceDlg::OnBtnSearch()
{
	...
	if (m_strJongCode.IsEmpty())
	{
		AfxMessageBox("종목코드를 입력해 주세요~!");
...
void CCurrentPriceDlg::OnBtnRealPause()
{
	CString strBtnText(_T("수신"));
	GetDlgItem(IDC_BTN_REALPAUSE)->GetWindowText(strBtnText);
	if(m_nRealPause == 0)
	{
		m_nRealPause = 1;
		strBtnText = _T("중지");
	...
	GetDlgItem(IDC_BTN_REALPAUSE)->SetWindowText(strBtnText);
}
...
void CCurrentPriceDlg::InitCurPrcGrid()
{
	...
	// 열의 넓이 설정
	int i, nWidth[] = {80, 20, 80, 60, 70, 80};
	...
	else if (strRQName == _T("체결틱"))			// 체결정보 설정
	{
		...
		for (i = 0; i < nCnt; i++)
		{
			...
			for (j = 0; j < nFieldCnt; j++)
			{
				...
				if (j == nFieldCnt - 1)
				{	// 백만단위
					strData.Format("%ld", long(_ttof(strData) / 1000000));
				}
				*/
				arrData.Add(strData);
			}
...
void CCurrentPriceDlg::OnReceiveRealDataKhopenapictrl(LPCTSTR sJongmokCode, LPCTSTR sRealType, LPCTSTR sRealData)
{
	...
	if (!strcmp(sRealType, "주식시세"))		// 주식시세
	{
		...
		{
			if (_ttoi(lstOPT10001[i].strFID) < 0)
			{
				arrData.Add("");
				continue;
			}
			strData = theApp.m_khOpenApi.GetCommRealData(sJongmokCode, _ttoi(lstOPT10001[i].strFID));	strData.Trim();
			arrData.Add(strData);
		}
	...
	else if (!strcmp(sRealType, "주식체결"))	// 주식체결
	{
		...
		for (i = 0; i < nFieldCnt; i++)
		{
			if (_ttoi(lstOPT10003[i].strFID) < 0)
			{
				arrData.Add("");
				continue;
			}
			strData = theApp.m_khOpenApi.GetCommRealData(sJongmokCode, _ttoi(lstOPT10003[i].strFID));	strData.Trim();
			arrData.Add(strData);
		}
		SetDataTickGrid(1, 1, arrData);// 시간, 현재가, 기호, 전일대비, 누적거래량, 누적거래대금
		...
		for (i = 0; i < nFieldCnt; i++)
		{
			if (_ttoi(lstOPT10001_B[i].strFID) < 0)
			{
				arrData.Add("");
				continue;
			}
			strData = theApp.m_khOpenApi.GetCommRealData(sJongmokCode, _ttoi(lstOPT10001_B[i].strFID));	strData.Trim();
			arrData.Add(strData);
		}
		SetDataJongInfoGrid(arrData, sRealType);
	}
			...
			!strcmp(sRealType, "주식시간외호가"))	// 주식시간외호가
	{
		...
		for (i = 0; i < nFieldCnt; i++)
		{
			if (_ttoi(lstOPT10004[i].strFID) < 0)
			{
				arrData.Add("");
				continue;
			}
			strData = theApp.m_khOpenApi.GetCommRealData(sJongmokCode, _ttoi(lstOPT10004[i].strFID));	strData.Trim();
			arrData.Add(strData);
		}
...
void CCurrentPriceDlg::SetDataTickGrid(int nInsertRow, int nRow, CStringArray &arrData)
{
	...
	for (i = 0; i < nCnt; i++)
	{
		...
		if (lstOPT10003[i].bTextColor)
		{
			...
			strTemp = arrData.GetAt(4);/// 체결량	
			theApp.SetDataFgColour(&m_grdTick, nRow, 4, strTemp);
		}
		...
		{
			BOOL bRet = m_grdTick.SetItemText(nRow, lstOPT10003[i].nCol, theApp.ConvDataFormat(lstOPT10003[i].nDataType, strData, lstOPT10003[i].strBeforeData, lstOPT10003[i].strAfterData));
		}
	}
	m_grdTick.Invalidate();
}
