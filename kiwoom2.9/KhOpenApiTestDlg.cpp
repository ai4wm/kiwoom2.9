	...
	while (pos != NULL)
	{
		...
		if (m_mapScreen.Lookup(strKey, (void *&)pWnd) && pWnd)
		{
			...
			case 3:		// 조건검색
				...
				break;
			default:
				TRACE(_T("default Real Condition\n"));
				break;
			}
		}
	}
...
{//  사용자 조건식 불러오기 요청에 대한 응답 수신시 발생되는 이벤트
	...
	CStringArray	arrScreenNum;
	GetScreenNums(3, arrScreenNum);	//조건검색

	for(auto i = 0; i < arrScreenNum.GetCount(); i++)
	{
		CWnd *pWnd = NULL;
		if (m_mapScreen.Lookup(arrScreenNum.GetAt(i), (void *&)pWnd) && pWnd)
		{
...
void CKhOpenApiTestDlg::OnEventConnect(LONG nItemCnt)
{
	if (nItemCnt == 0)
	{
		...
		CString		strID = theApp.m_khOpenApi.GetLoginInfo(_T("USER_ID"));
		CString		strTitle;
		GetWindowText(strTitle);
		strMsg.Format(_T(" ( %s )"), strID);
		strTitle += strMsg;
		SetWindowText(strTitle);
	}
	else
...
BOOL CKhOpenApiTestDlg::GetNextScreenNum(int nScreenType)
{
	...
	CString strScreenNum, strTemp;
	strScreenNum.Format("%04d", m_nScrN0);
	if (m_mapScreenNum.Lookup(strScreenNum, strTemp))
	{
	...
	m_mapScreenNum.SetAt(strScreenNum, strTemp);
	return TRUE;
}

void CKhOpenApiTestDlg::GetScreenNums(int nScreenType, CStringArray &rArrScreen)
{
	POSITION pos = m_mapScreenNum.GetStartPosition();
	CString	strScreenNum, strSaved, strScreenType;
	strScreenType.Format(_T("%d"), nScreenType);
	while (pos != NULL)
	{
		m_mapScreenNum.GetNextAssoc(pos, strScreenNum, strSaved);
		if(strScreenType == strSaved)
		{
			rArrScreen.Add(strScreenNum);
		}
	}
}
...
{// 종목정보, 업종정보
	...
	CString strRet0 = theApp.m_khOpenApi.GetMasterStockState(strTemp); /// 종목정보
	CString strRet1 = theApp.m_khOpenApi.KOA_Functions(_T("GetMasterStockInfo"), strTemp);/// 업종정보
	CString strRet;
	strRet.Format(_T("종목정보 = %s \r\n종목업종 = %s"), strRet0, strRet1);
