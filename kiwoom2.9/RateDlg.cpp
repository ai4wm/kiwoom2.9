...
#define  COMMISSION1	0.0000	// 제세금(코스피)
#define  COMMISSION2	0.0015	// 제세금(코스닥)
#define  COMMISSION3	0.0015	// 제세금(코스피 농어촌특별세)
#define  COMMISSION4	0.0015	// 제세금(K-OTC)
...
const stGRID lstFID[] = 
{
	...
	{_T("평가손익"),	_T("-1"),	-1,	6,	DT_NUMBER_SIGN,	FALSE,	DT_RIGHT,	_T(""),	_T("")},
	{_T("수익률"),	_T("-1"),	-1,	7,	DT_NUMBER_SIGN,	FALSE,	DT_RIGHT,	_T(""),	_T("")},
...
BOOL CRateDlg::OnInitDialog()
{
	...
	::GetPrivateProfileString(_T("RATEDLG"), _T("제세금_코스피"), _T(""), szItem, nSize, strFileName);/// 제세금(코스피)
	strComm1 = szItem;
	if(strComm1.IsEmpty())
	{
		::GetPrivateProfileString(_T("RATEDLG"), _T("COMMISSION1"), _T(""), szItem, nSize, strFileName);/// 제세금(코스피)
		strComm1 = szItem;
	}

	::GetPrivateProfileString(_T("RATEDLG"), _T("제세금_코스닥"), _T(""), szItem, nSize, strFileName);/// 제세금(코스닥)
	strComm2 = szItem;
	if(strComm2.IsEmpty())
	{
		::GetPrivateProfileString(_T("RATEDLG"), _T("COMMISSION2"), _T(""), szItem, nSize, strFileName);/// 제세금(코스닥)
		strComm2 = szItem;
	}

	::GetPrivateProfileString(_T("RATEDLG"), _T("제세금_농특세"), _T(""), szItem, nSize, strFileName);/// 제세금(농어촌특별세)
	strComm3 = szItem;
	if(strComm3.IsEmpty())
	{
		::GetPrivateProfileString(_T("RATEDLG"), _T("COMMISSION3"), _T(""), szItem, nSize, strFileName);/// 제세금(농어촌특별세)
		strComm3 = szItem;
	}

	::GetPrivateProfileString(_T("RATEDLG"), _T("제세금_K-OTC"), _T(""), szItem, nSize, strFileName);	/// 제세금(K-OTC)
	strComm4 = szItem;
	if(strComm4.IsEmpty())
	{
		::GetPrivateProfileString(_T("RATEDLG"), _T("COMMISSION4"), _T(""), szItem, nSize, strFileName);/// 제세금(K-OTC)
		strComm4 = szItem;
	}
	...
	CString strMsg;
	strMsg.Format(_T("수수료: %.3f%%, 거래세 코스피: %.2f%%, 코스닥: %.2f%%, 농특세: %.2f%%")
		, m_dFee * 100
		, m_dComm1 * 100
		, m_dComm2 * 100
		, m_dComm3 * 100
	...
	m_cbExchange.AddString(_T("통합"));// 통합:_AL
	m_cbExchange.AddString(_T("KRX"));// KRX:공백
	m_cbExchange.AddString(_T("NXT"));// NXT:_NX
	m_cbExchange.SetCurSel(0);

	m_strExchageCode = _T("_AL");// 거래소별 종목코드 구성 통합:_AL, KRX:공백, NXT:_NX, ex) 039490_AL, 039490, 039490_NX
...
{// 조회요청 수신 이벤트
	...
	if (strRQName == _T("계좌수익률"))			// 계좌수익률
	{
		...
		if (strPrevNext == _T("2"))	//연속조회
		{
			...
			theApp.m_khOpenApi.SetInputValue(_T("거래소구분"), strExchType);
...
{// 종목별 수익률 계산
	...
	//종목별 평가손익 = 평가금액 - 매입금액 - 매수가계산 수수료 - 매도가계산 수수료 - 제세금 가계산
