...
const stGRID lstOPTKWFID[] = 
{
	...
	{_T("등락율"),		_T("12"),	-1,	5,	DT_ZERO_NUMBER,	TRUE,	DT_RIGHT,	_T(""),	_T("%")}, 
	{_T("거래량"),		_T("13"),	-1,	6,	DT_ZERO_NUMBER,	FALSE,	DT_RIGHT,	_T(""),	_T("")}, 
	{_T("전일거래량대비"),	_T("30"),	-1,	7,	DT_ZERO_NUMBER,	TRUE,	DT_RIGHT,	_T(""),	_T("%")}, 
};
...
BOOL CKwanSimDlg::OnInitDialog()
{
	...
	((CEdit*)GetDlgItem(IDC_EDT_JONGCODE))->SetLimitText(10);	// 종목코드 최대길이 설정
...
void CKwanSimDlg::OnBtnJongAdd()
{
	...
	if (strCode.IsEmpty())
	{
		AfxMessageBox(_T("종목코드를 입력 해 주세요~!"));
